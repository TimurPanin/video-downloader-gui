    ...
    def test_youtube_urls(self):
        ...
        for url in valid_urls:
            with self.subTest(url=url):
                ...
                is_valid, _ = Validation.validate_url_for_service(url, 'youtube')
    ...
    def test_tiktok_urls(self):
        ...
        for url in valid_urls:
            with self.subTest(url=url):
                ...
                is_valid, _ = Validation.validate_url_for_service(url, 'tiktok')
                self.assertTrue(is_valid)
...
class TestConfig(unittest.TestCase):
    ...
    def setUp(self):
        """Используем отдельный временный config-файл для каждого теста."""
        self.temp_dir = tempfile.TemporaryDirectory()
        self.config_path = Path(self.temp_dir.name) / 'settings.json'
